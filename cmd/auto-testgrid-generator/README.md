...
## Deployment
Periodic Prow job ([recent runs](https://prow.ci.openshift.org/?job=periodic-prow-auto-testgrid-generator)). The container image includes both the `auto-testgrid-generator` and `testgrid-config-generator` binaries (the latter at `/usr/bin/testgrid-config-generator`). The job runs with a GitHub token for PR creation against `kubernetes/test-infra`.
