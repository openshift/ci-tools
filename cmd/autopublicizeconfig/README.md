...
## Deployment
Periodic Prow job ([recent runs](https://prow.ci.openshift.org/?job=periodic-auto-publicize-config)). Creates PRs against the `openshift/release` repository on branch `auto-publicize-sync`.
