...
func gatherOptions() options {
	...
	if err := fs.Parse(os.Args[1:]); err != nil { // todo: should this be args[1:] ?
...
func newValidator() *profileValidator {
...
func main() {
	...
	// TODO after they are implemented into the config file, add checks for cluster type and leases
...
func (validator *profileValidator) loadConfig(configPath string) error {
	...
	if err = yaml.Unmarshal(configContents, &profilesList); err != nil {
		return fmt.Errorf("failed to unmarshall file %v. Please check that the formatting in the file is correct. Full error: %w", configPath, err)
	}
