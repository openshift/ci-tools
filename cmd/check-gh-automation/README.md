...
## Deployment
CLI tool. Typically run as a presubmit job on openshift/release PRs (with `--candidate-path` pointing to the PR checkout) and as a periodic job ([recent runs](https://prow.ci.openshift.org/?job=periodic-check-gh-automation)) checking all Prow-configured repos. Requires a GitHub token with read access to check org membership, collaborator status, app installation, and permissions.
