...
func main() {
	...
	opt.metricsController = metrics.NewMetricsController()
	go opt.metricsController.Run()
...
func (o *options) Run() []error {
	...
	buildSteps, promotionSteps, err := defaults.FromConfig(ctx, o.configSpec, &o.graphConfig, o.jobSpec, o.templates, o.writeParams, o.promote, o.clusterConfig,
		...
		o.nodeName, nodeArchitectures, o.targetAdditionalSuffix, o.manifestToolDockerCfg, o.localRegistryDNS, streams, injectedTest, o.enableSecretsStoreCSIDriver, o.metricsController)
	if err != nil {
	...
	o.metricsController.Record(&metrics.InsightsEvent{Name: "namespace_created", AdditionalContext: map[string]any{"namespace": o.namespace}})
	...
	return interrupt.New(handler, o.saveNamespaceArtifacts).Run(func() []error {
		...
		if len(errs) > 0 {
			o.metricsController.Record(&metrics.InsightsEvent{Name: "ci_operator_failed"})
		...
		for i := 0; i < lenOfPromotionSteps; i++ {
			...
			case err := <-errChan:
				...
				o.metricsController.Record(&metrics.InsightsEvent{Name: "post_step_failed", AdditionalContext: map[string]any{"error": errorDesc}})
		...
		o.metricsController.Record(&metrics.InsightsEvent{Name: "ci_operator_succeeded"})
		return nil
	})
}
