...
// It checks for environment variables that start with "OVERRIDE_" and applies them to the environment settings of each test.
func applyEnvOverrides(o *options) {
	for _, envVar := range os.Environ() {
		...
		if !strings.HasPrefix(key, "MULTISTAGE_PARAM_OVERRIDE_") {
			continue
		}
		for _, test := range o.configSpec.Tests {
			if test.MultiStageTestConfigurationLiteral != nil {
