...
## Deployment
CLI tool. Requires a running Dgraph instance with the appropriate GraphQL schema deployed. Typically run as a periodic Prow job ([recent runs](https://prow.ci.openshift.org/?job=periodic-openshift-release-image-graph-generator)) with a local checkout of openshift/release.
