...
type diffProcessorFunc func(l *logrus.Entry, org, repo, path string, oldContent, newContent []byte) error
...
func (dp *diffProcessor) addDiff(l *logrus.Entry, org, repo, path string, oldContent, newContent []byte) error {
	dp.lock.Lock()
	defer dp.lock.Unlock()
	dp.diffs = append(dp.diffs, diff{log: l, org: org, repo: repo, path: path, oldContent: oldContent, newContent: newContent})
	return nil
}
	...
	for _, d := range dp.diffs {
		...
		if err := func(d diff) error {
			...
			if err := dp.prCreationOpts.UpsertPR(
				...
				d.repo,
				"master",
				fmt.Sprintf("Updating %s baseimages to mach ocp-build-data config", d.path),
				strings.Join([]string{
