...
func makeProwjob(configuration *api.ReleaseBuildConfiguration, pr *github.PullRequest, base *api.Metadata, inject *api.MetadataWithTest, prowConfigPath string) *config.Periodic {
	...
	for i := range configuration.Tests {
		if configuration.Tests[i].As == inject.Test {
			...
			// TODO(muller): Avoid sharing when we run the same job multiple times.
			// TOOD(muller): In theory we could be shared if we called the injected test somethign safe? Then the individual executions could share e.g. image builds.
			jobBaseGen.PodSpec.Add(prowgen.CustomHashInput(time.Now().String()))
			// TODO(muller): Refactor this bit of Prowgen into something nicer
	...
	// TODO(muller): Prow decorates jobs on disk load and the relevant code is internal
	// Don't laugh
	tmpconfig := &config.JobConfig{Periodics: []config.Periodic{*periodic}}
	file, err := ioutil.TempFile("", "*.yaml")
	if err != nil {
		panic("Failed to create a temporary file")
	}
	defer os.Remove(file.Name())
	if err := jobconfig.WriteToFile(file.Name(), tmpconfig); err != nil {
		panic("Failed to roundtrip Prow config: write")
	}

	decoratedConfig, err := config.Load(prowConfigPath, file.Name(), nil, "")
	if err != nil {
		panic("Failed to roundtrip Prow config: read")
	}

	return &decoratedConfig.JobConfig.Periodics[0]
}
...
func main() {
	...
	prowjobConfig := makeProwjob(config, pr, base, inject, o.prowConfigPath)
	if err != nil {
