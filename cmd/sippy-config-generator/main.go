...
func main() {
	...
	if o.customizationFile != "" {
		...
		if err != nil {
			logrus.Fatalf("could not read customization file: %w", err)
		}

		if err := yaml.Unmarshal(data, &sippyConfig); err != nil {
			logrus.Fatalf("could not unmarshal customization file: %w", err)
		}
	}
	...
	if err != nil {
		logrus.Fatalf("could not marshal config: %w", err)
	}
