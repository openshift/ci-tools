...
## Deployment
Runs as a periodic CronJob on app.ci ([recent runs](https://prow.ci.openshift.org/?job=periodic-sprint-automation)). Two instances: one daily (all activities), one on Mondays only (with `--week-start`).
