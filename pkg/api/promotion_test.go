	...
	testCases := []struct {
		...
		},
		{
			...
			expected: map[string]string{
				...
				"ocp/ovn-kubernetes:ovn-kubernetes":                             "quay-proxy.ci.openshift.org/openshift/ci@sha256:def456",
			},
		},
		{
		...
		{
			name:   "tag-only pipeline source",
			source: "registry.build02.ci.openshift.org/ci-op-abc/pipeline:some-tag",
			target: "quay.io/openshift/ci:ocp_4.22_ovn-kubernetes",
			tag: ImageStreamTagReference{
				Namespace: "ocp",
				Name:      "4.22",
