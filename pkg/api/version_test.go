...
func TestGetAllPreviousVersions(t *testing.T) {
	availableVersions := []string{"4.18", "4.19", "4.20", "4.21", "4.22", "4.23", "5.0", "5.1", "5.2"}
	...
	testCases := []struct {
		...
		{
			name:              "5.0 returns override (4.22)",
			version:           "5.0",
			availableVersions: availableVersions,
			expected:          []string{"4.22"},
		},
		{
			name:              "5.1 returns natural progression (5.0)",
			version:           "5.1",
			availableVersions: availableVersions,
			expected:          []string{"5.0"},
		},
		{
			name:              "4.22 returns natural progression (4.21)",
			version:           "4.22",
			availableVersions: availableVersions,
			expected:          []string{"4.21"},
		},
		{
			name:              "6.0 finds highest 5.x from available",
			version:           "6.0",
			availableVersions: availableVersions,
			expected:          []string{"5.2"},
		},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := GetAllPreviousVersions(tc.version, tc.availableVersions)
			if tc.expectError {
				if err == nil {
					t.Errorf("expected error for version %s, got result %v", tc.version, result)
				}
			} else {
				if err != nil {
					t.Errorf("unexpected error for version %s: %v", tc.version, err)
				...
				if len(result) != len(tc.expected) {
					t.Errorf("GetAllPreviousVersions(%s) = %v, expected %v", tc.version, result, tc.expected)
				} else {
					for i, v := range result {
						if v != tc.expected[i] {
							t.Errorf("GetAllPreviousVersions(%s)[%d] = %s, expected %s", tc.version, i, v, tc.expected[i])
						}
					}
				}
			}
...
func TestGetAllPreviousVersionsSimple(t *testing.T) {
	testCases := []struct {
		name        string
		version     string
		expected    []string
		...
		{
			name:        "6.0 fails without override",
			version:     "6.0",
			expectError: true,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := GetAllPreviousVersionsSimple(tc.version)
			if tc.expectError {
				if err == nil {
					t.Errorf("expected error for version %s, got result %v", tc.version, result)
				}
				...
				if len(result) != len(tc.expected) {
					t.Errorf("GetAllPreviousVersionsSimple(%s) = %v, expected %v", tc.version, result, tc.expected)
				} else {
					for i, v := range result {
						if v != tc.expected[i] {
							t.Errorf("GetAllPreviousVersionsSimple(%s)[%d] = %s, expected %s", tc.version, i, v, tc.expected[i])
						}
					}
				}
			}
