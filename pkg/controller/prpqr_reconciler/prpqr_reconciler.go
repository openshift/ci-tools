...
func (r *reconciler) generateProwjob(ciopConfig *api.ReleaseBuildConfiguration,
	...
	extras := api.NewProwgenExtras(api.Prowgen{}, ciopConfig)
	...
	for i := range ciopConfig.Tests {
		...
		jobBaseGen := prowgen.NewProwJobBaseBuilderForTest(ciopConfig, extras, prowgen.NewCiOperatorPodSpecGenerator(), test)
		...
		periodic = prowgen.GeneratePeriodicForTest(jobBaseGen, extras, prowgen.FromConfigSpec(ciopConfig), func(options *prowgen.GeneratePeriodicOptions) {
			options.Cron = "@yearly"
		})
...
func generateAggregatorJob(baseCiop *api.Metadata, uid, aggregatorJobName, jobName, prpqrName, prpqrNamespace string, getCfg prowConfigGetter, startTime time.Time, submitted string, aggregatedJobTimeout time.Duration) (*prowv1.ProwJob, error) {
	...
	jobBaseGen := prowgen.NewProwJobBaseBuilderForTest(ciopConfig, &api.ProwgenExtras{}, prowgen.NewCiOperatorPodSpecGenerator(), ciopConfig.Tests[0])

	periodic := prowgen.GeneratePeriodicForTest(jobBaseGen, &api.ProwgenExtras{}, prowgen.FromConfigSpec(ciopConfig), func(options *prowgen.GeneratePeriodicOptions) {
		options.Cron = "@yearly"
	})
