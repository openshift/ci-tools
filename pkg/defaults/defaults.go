...
func fromConfig(ctx context.Context, cfg *Config) ([]api.Step, []api.Step, error) {
	...
	for _, rawStep := range rawSteps {
		if testStep := rawStep.TestStepConfiguration; testStep != nil {
			buildSteps = append(buildSteps, stepForTest(cfg, inputImages, testStep, &imageConfigs)...)
			continue
		}
...
func stepForTest(cfg *Config, inputImages inputImageSet, c *api.TestStepConfiguration, imageConfigs *[]*api.InputImageTagStepConfiguration,
	...
	if test := c.MultiStageTestConfigurationLiteral; test != nil {
		...
		if len(leases) != 0 {
			step = steps.IPPoolStep(cfg.LeaseClient, cfg.podClient, step, params, cfg.JobSpec.Namespace, cfg.MetricsAgent, *test.ClusterProfileDetails, cfg.CIConfig.Metadata.Branch)
