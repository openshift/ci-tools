...
func fromConfig(
	...
	for _, rawStep := range rawSteps {
		if testStep := rawStep.TestStepConfiguration; testStep != nil {
			steps, testHasReleaseStep, err := stepForTest(ctx, config, params, podClient, leaseClient, templateClient, client, hiveClient, jobSpec, inputImages, testStep, &imageConfigs, pullSecret, censor, nodeName, targetAdditionalSuffix, waitObservers)
			if err != nil {
				return nil, nil, err
			}
...
func stepForTest(
	...
	if test := c.MultiStageTestConfigurationLiteral; test != nil {
		...
		step := multi_stage.MultiStageTestStep(*c, config, params, podClient, jobSpec, leases, nodeName, targetAdditionalSuffix, waitObservers)
		if len(leases) != 0 {
			step = steps.LeaseStep(leaseClient, leases, step, jobSpec.Namespace)
			addProvidesForStep(step, params)
		}
