...
func TestAggregateTestCasePropagatesLifecycle(t *testing.T) {
	...
	err := aggregateTestCase("suite", combined, "logs/job", "test-platform-results-public", "run1", source)
	assert.NoError(t, err)
	...
	err = aggregateTestCase("suite", combined, "logs/job", "test-platform-results-public", "run2", source2)
	assert.NoError(t, err)
	assert.Equal(t, "informing", combined.Lifecycle)
}
