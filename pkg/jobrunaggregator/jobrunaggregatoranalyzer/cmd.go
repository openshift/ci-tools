...
func (f *JobRunsAnalyzerFlags) BindFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&f.GCSBucket, "google-storage-bucket", jobrunaggregatorlib.DefaultGCSBucket, "The optional GCS Bucket holding test artifacts")
}
...
func (f *JobRunsAnalyzerFlags) ToOptions(ctx context.Context) (*JobRunAggregatorAnalyzerOptions, error) {
	...
	return &JobRunAggregatorAnalyzerOptions{
		...
		passFailCalculator:      newWeeklyAverageFromTenDaysAgo(f.JobName, estimatedStartTime, 5, ciDataClient),
		jobName:                 f.JobName,
