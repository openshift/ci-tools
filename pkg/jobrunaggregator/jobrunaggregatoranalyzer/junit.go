...
func (a *aggregatedJobRunJunit) aggregateAllJobRuns() (*junit.TestSuites, error) {
	...
	for _, aggregationName := range sets.StringKeySet(a.aggregationNameToJobRuns).List() {
		...
		for _, currJobRunJunit := range jobRunJunits {
			if err := combineTestSuites(combined, a.jobGCSBucketRoot, a.gcsBucket, currJobRunJunit.jobRun.GetJobRunID(), currJobRunJunit.combinedJunit); err != nil {
				return nil, err
			}
		}
	}
...
func combineTestSuites(combined *junit.TestSuites, jobGCSBucketRoot, gcsBucket, toAddJobRunID string, toAdd *junit.TestSuites) error {
	for _, suiteToAdd := range toAdd.Suites {
		combinedSuite := ensureSuiteInSuites(combined, suiteToAdd.Name)
		if err := combineTestSuite([]string{}, combinedSuite, jobGCSBucketRoot, gcsBucket, toAddJobRunID, suiteToAdd); err != nil {
			return err
		}
	}
	return nil
...
func combineTestSuite(parentSuiteNames []string, combined *junit.TestSuite, jobGCSBucketRoot, gcsBucket, toAddJobRunID string, toAdd *junit.TestSuite) error {
	...
	for _, testCaseToAdd := range toAdd.TestCases {
		...
		if err := aggregateTestCase(suiteAsSingleString, combinedTestCase, jobGCSBucketRoot, gcsBucket, toAddJobRunID, testCaseToAdd); err != nil {
			return err
		}
	}
	...
	for _, suiteToAdd := range toAdd.Children {
		...
		if err := combineTestSuite(currentSuiteNames, combinedSuite, jobGCSBucketRoot, gcsBucket, toAddJobRunID, suiteToAdd); err != nil {
			return err
		}
	}
...
func aggregateTestCase(testSuiteName string, combined *junit.TestCase, jobGCSBucketRoot, gcsBucket, toAddJobRunID string, toAdd *junit.TestCase) error {
