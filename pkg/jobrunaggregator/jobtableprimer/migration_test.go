...
func TestMigrationOfJobAttributes(t *testing.T) {
	previousJobsToAnalyze := []jobrunaggregatorapi.JobRow{
		{
	...
	for _, mustExist := range previousJobsToAnalyze {
		found := false
		for _, curr := range jobsToAnalyze {
			if reflect.DeepEqual(mustExist, curr) {
				found = true
				break
			}
		}
