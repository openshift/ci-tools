...
func TestMachinesPlugin_Record(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "captures snapshot of all workloads",
			autoscalers: []autoscalingv1beta1.MachineAutoscaler{
				{
			...
			event: &MachinesEvent{Workload: "tests"},
			expected: []MetricsEvent{
				&MachinesEvent{
					Workload:      "tests",
					TotalMachines: 3,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.expected, events, cmpopts.IgnoreFields(MachinesEvent{}, "Timestamp")); diff != "" {
