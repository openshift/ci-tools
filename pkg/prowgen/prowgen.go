...
func GenerateJobs(configSpec *cioperatorapi.ReleaseBuildConfiguration, info *ProwgenInfo) (*prowconfig.JobConfig, error) {
	...
	// ci-operator config can also disable all rehearsals
	disableAllRehearsals := rehearsals.DisableAll || (configSpec.Prowgen != nil && configSpec.Prowgen.DisableRehearsals)
	...
	for _, element := range configSpec.Tests {
		...
		for i := 1; i <= shardCount; i++ {
			...
			disableRehearsal := disableAllRehearsals || disabledRehearsals.Has(element.As) || element.DisableRehearsal
	...
	skipOperatorPresubmits := info.Config.SkipPresubmits(configSpec.Metadata.Branch, configSpec.Metadata.Variant) ||
		(configSpec.Operator != nil && configSpec.Operator.SkipPresubmits)
