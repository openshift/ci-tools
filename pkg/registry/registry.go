...
// Load takes the path to a registry config directory and returns the full set of references, chains,
// and workflows that the registry's Resolver needs to resolve a user's MultiStageTestConfiguration
func Load(root string, flags RegistryFlag) (ReferenceByName, ChainByName, WorkflowByName, map[string]string, api.RegistryMetadata, ObserverByName, error) {
	flat := flags&RegistryFlat != 0
	references := ReferenceByName{}
	chains := ChainByName{}
	...
	if _, err = NewGraph(references, chains, workflows, observers); err != nil {
		return nil, nil, nil, nil, nil, nil, err
	}
	err = Validate(references, chains, workflows, observers)
	if err != nil {
		return nil, nil, nil, nil, nil, nil, err
	}
