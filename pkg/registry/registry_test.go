...
func TestLoad(t *testing.T) {
	...
	var (
		expectedReferences = ReferenceByName{
		...
		expectedWorkflows = WorkflowByName{
			"ipi": {
				Pre: []api.TestStep{{
					Chain: &installChain,
				}},
		...
		expectedObservers = ObserverByName{
			"resourcewatcher": {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			references, chains, workflows, _, _, observers, err := Load(testCase.registryDir, testCase.flags)
	...
	_, _, _, _, _, _, err = Load(temp, RegistryFlag(0))
	if err == nil {
