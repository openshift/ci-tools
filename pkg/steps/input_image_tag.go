...
func (s *inputImageTagStep) run(ctx context.Context) error {
	...
	if s.config.ExternalImage != nil {
		...
		if s.config.ExternalImage.PullSecret != "" {
			// Get the secret from the test-credentials namespace. We expect it exists
			ciSecret := &coreapi.Secret{}
			err := s.client.Get(ctx, ctrlruntimeclient.ObjectKey{Namespace: "test-credentials", Name: s.config.ExternalImage.PullSecret}, ciSecret)
			if err != nil {
				return fmt.Errorf("failed to get secret '%s' from test-credentials namespace: %w", s.config.ExternalImage.PullSecret, err)
			}
			...
			newSecret := &coreapi.Secret{
				...
				ObjectMeta: metav1.ObjectMeta{
					Name:      "external-pull-secret",
					Namespace: s.jobSpec.Namespace(),
				},
			}

			if err := s.client.Create(ctx, newSecret); err != nil {
				return fmt.Errorf("failed to create secret '%s' in '%s' namespace: %w", newSecret.Name, newSecret.Namespace, err)
			}
		}
