...
func TestGeneratePods(t *testing.T) {
	...
	step := newMultiStageTestStep(config.Tests[0], &config, nil, nil, nil, nil, "artifact_dir", &jobSpec)
	env := []coreapi.EnvVar{
...
func TestGeneratePodsEnvironment(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			}, &api.ReleaseBuildConfiguration{}, nil, nil, nil, nil, "", &jobSpec)
...
func TestRun(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			}, &api.ReleaseBuildConfiguration{}, nil, &fakePodClient{NewPodClient(client, nil, nil)}, client, crclient, "", &jobSpec)
...
func TestArtifacts(t *testing.T) {
	...
	}, &api.ReleaseBuildConfiguration{}, nil, &fakePodClient{NewPodClient(client, nil, nil)}, client, fakectrlruntimeclient.NewFakeClient(), tmp, &jobSpec)
...
func TestJUnit(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			}, &api.ReleaseBuildConfiguration{}, nil, &fakePodClient{NewPodClient(client, nil, nil)}, client, fakectrlruntimeclient.NewFakeClient(), "/dev/null", &jobSpec)
			if err := step.Run(context.Background()); tc.failures == nil && err != nil {
				t.Error(err)
				return
			}
