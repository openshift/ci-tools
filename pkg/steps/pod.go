...
func (s *podStep) ResolveMultiArch() sets.Set[string] {
	...
	if s.config.NodeArchitecture != "" {
		arch = string(s.config.NodeArchitecture)
	}
...
func (s *podStep) generatePodForStep(image string, containerResources coreapi.ResourceRequirements, clone bool) (*coreapi.Pod, error) {
	...
	if len(s.resolvedGSMCredentials) > 0 {
		...
		collectionMountGroups := csi_secrets.GroupCredentialsByMountPath(s.resolvedGSMCredentials)
...
func (s *podStep) resolveAndCreateGSMSecrets(ctx context.Context) error {
	...
	if err := csi_secrets.ValidateNoFileCollisionsOnMountPath(resolved); err != nil {
