...
func (s *releaseImagesTagStep) run(ctx context.Context) error {
	...
	newIS, err := snapshotStream(ctx, s.client, s.config.Namespace, s.config.Name, s.jobSpec.Namespace, api.LatestReleaseName, s.integratedStream, s.config.ReferencePolicy)
	if err != nil {
		return err
	}
	...
	for _, tag := range s.integratedStream.Tags {
		spec, ok, _ := util.ResolvePullSpec(newIS, tag, false, s.client.MetricsAgent())
		if !ok {
