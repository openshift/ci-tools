...
func (r *releaseSnapshotStep) run(ctx context.Context) error {
	...
	if r.config.ReferencePolicy != nil {
		refPolicy = *r.config.ReferencePolicy
	}
	_, err := snapshotStream(ctx, r.client, r.config.Namespace, r.config.Name, r.jobSpec.Namespace, r.name, r.integratedStream, refPolicy)
	return err
}
