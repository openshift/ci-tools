...
END
	...
	if err := waitForDeployment(ctx, s.client, deployment.Name); err != nil {
...
func waitForRouteReachable(ctx context.Context, client ctrlruntimeclient.Client, name, scheme string, pathSegments ...string) error {
	host, err := admittedHostForRoute(client, name, 5*time.Minute)
	if err != nil {
...
func (s *rpmServerStep) rpmRepoURL() (string, error) {
	host, err := admittedHostForRoute(s.client, RPMRepoName, time.Minute)
	if err != nil {
...
func admittedHostForRoute(client ctrlruntimeclient.Client, name string, timeout time.Duration) (string, error) {
	var repoHost string
	if err := wait.PollImmediate(time.Second, timeout, func() (bool, error) {
		route := &routev1.Route{}
		if err := client.Get(context.TODO(), ctrlruntimeclient.ObjectKey{Name: name}, route); err != nil {
