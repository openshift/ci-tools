...
func ResolvePullSpec(is *imageapi.ImageStream, tag string, requireExact bool, metricsAgent *metrics.MetricsAgent) (string, bool, imageapi.TagEventCondition) {
	...
	metricsAgent.Record(&metrics.TagImportEvent{
		...
		ImageStreamName:   is.Name,
		TagName:           tag,
		FullTagName:       is.Namespace + "/" + is.Name + ":" + tag,
		SourceImage:       pullSpec,
		SourceImageKind:   "ImageStreamTag",
		Success:           true,
		AdditionalContext: map[string]any{"operation_type": "cached"},
	})
...
func CreateImageStreamWithMetrics(ctx context.Context, client ctrlruntimeclient.Client, imageStream *imageapi.ImageStream, metricsAgent *metrics.MetricsAgent) (*imageapi.ImageStream, error) {
	...
	if err := client.Create(ctx, imageStream); err != nil {
		if !kerrors.IsAlreadyExists(err) {
			metricsAgent.RecordImageStreamEvent(imageStream.Namespace, imageStream.Name, false, err.Error())
	...
	metricsAgent.RecordImageStreamEvent(imageStream.Namespace, imageStream.Name, created)
