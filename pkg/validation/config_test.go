...
func TestValidateImageConfiguration(t *testing.T) {
	var testCases = []struct {
		...
		output: []error{
			errors.New("images: `run_if_changed` and `skip_if_only_changed` are mutually exclusive"),
		},
	}, {
		...
		output: []error{
			errors.New("images: `pipeline_run_if_changed` and `pipeline_skip_if_only_changed` are mutually exclusive"),
		},
	}, {
		...
		name: "run_if_changed with pipeline_skip_if_only_changed is valid",
		input: api.ImageConfiguration{
			RunIfChanged:              `^src/`,
			PipelineSkipIfOnlyChanged: `^docs/`,
		},
