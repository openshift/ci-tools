...
os::cmd::expect_success "ci-operator ${namespace} --registry ${suite_dir}/step-registry --config ${suite_dir}/config.yaml --lease-server http://localhost:8080 --lease-server-password-file /dev/null --lease-acquire-timeout 2s --target configurable-leases-count --secret-dir ${cluster_profiles}/success-cluster-profile"
